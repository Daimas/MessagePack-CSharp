...
echo on
start "Unity PACKAGE" /wait "c:\Program Files\unity\editor\Unity.exe" -batchmode -quit -nographics -silent-crashes -noUpm -buildTarget standalone -projectPath "%~dp0\" -executeMethod PackageExporter.Export -logfile "%LogDirectory%\unity_package.log"
@IF ERRORLEVEL 1 (
